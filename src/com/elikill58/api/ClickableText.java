	...
	static class MessageComponent {
			...
			try {
				if (Version.getVersion().isNewerOrEquals(Version.V1_8)) {
					...
					for (Object obj : compile()) {
						...
						Object packetToSend = PacketUtils.getNmsClass("PacketPlayOutChat").getConstructor(PacketUtils.getNmsClass("IChatBaseComponent")).newInstance(obj);
						PacketUtils.sendPacket(p, packetToSend);
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
		...
		public Object[] compile() {
			try {
				...
				GameAPI.GAME_PROVIDER.getLogger().severe(
